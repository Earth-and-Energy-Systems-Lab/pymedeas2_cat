...
@component.add(
    name="agricultural_land_pc",
    units="Ha/(person)",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"agricultural_land": 1, "mha_to_ha": 1, "population": 1},
)
def agricultural_land_pc():
    """
    Agricultural land per cápita.
    """
    return zidz(agricultural_land() * mha_to_ha(), population())


@component.add(
    name="agricultural_land_pc_until_2015",
    units="Ha/person",
    comp_type="Stateful",
    comp_subtype="SampleIfTrue",
    depends_on={"_sampleiftrue_agricultural_land_pc_until_2015": 1},
    other_deps={
        "_sampleiftrue_agricultural_land_pc_until_2015": {
            "initial": {"agricultural_land_pc": 1},
            "step": {"time": 1, "agricultural_land_pc": 1},
        }
    },
)
...
@component.add(
    name="aux_reach_ag_land",
    units="Dmnl",
    comp_type="Auxiliary",
    comp_subtype="with Lookup",
    depends_on={"agricultural_land": 1},
)
...
@component.add(
    name="aux_reach_available_forest",
    units="Dmnl",
    comp_type="Auxiliary",
    comp_subtype="with Lookup",
    depends_on={"available_forest_area": 1},
)
...
def forest_stock_ratio():
    ...
    Forest stock ratio.
    """
    return 1 / (growing_stock_forest_per_ha() * mha_to_ha() * wood_energy_density())
...
def hist_variation_urban_land():
    ...
    return if_then_else(
        time() < 2014,
        lambda: (
            historic_urban_land(time() + time_step()) - historic_urban_land(time())
        )
        / time_step(),
        lambda: 0,
    )
...
@component.add(
    name="MHa_to_Ha", units="Ha/MHa", comp_type="Constant", comp_subtype="Normal"
)
def mha_to_ha():
    return 1000000.0
...
@component.add(
    name='"scarcity_agricultural_land?"',
    units="Dmnl",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
...
def scarcity_agricultural_land():
    ...
    return if_then_else(
        ...
        lambda: 0,
        lambda: if_then_else(
            agricultural_land_pc()
            > agricultural_land_pc_until_2015() * threshold_scarcity_ag_land(),
            lambda: 1,
            lambda: 0,
        ),
    )
...
@component.add(
    name="threshold_scarcity_ag_land",
    units="Dmnl",
    comp_type="Constant",
    comp_subtype="Normal",
)
...
@component.add(
    name="Total_CAT_land_endogenous",
    units="MHa",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
...
def urban_land_rate():
    """
    Rate of urban surface rate. IF THEN ELSE(Time<2014, hist variation urban land, 0.000795116*urban land density variation+2.03205*1e-08*pop variation/"1 year")*aux reach ag land
    """
    return (
        if_then_else(
            time() < 2014,
